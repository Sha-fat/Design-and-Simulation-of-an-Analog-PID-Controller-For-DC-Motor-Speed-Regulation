...
* **`PSpice/`**: Contains the standalone schematic files (`.sch`) for the three test setups:
  * `PID_MotorSpeed.sch`: Baseline transient step response analysis.
  * `PID_MotorSpeed_param.sch`: Parametric sweep analysis for controller tuning and variation.
  * `PID_MotorSpeed_Disturbance rejection.sch`: Load torque step disturbance rejection analysis.
...
1. **Differential Error Amplifier**: Compares the reference speed setpoint against the tachometer feedback voltage.
2. **Analog PID Controller**:
   * **Proportional Stage ($K_p$)**: Provides immediate proportional correction.
   * **Integral Stage ($K_i$)**: Eliminates steady-state speed error.
   * **Derivative Stage ($K_d$)**: Improves phase margin and transient damping.
3. **Power Stage Driver**: Push-pull transistor driver feeding the motor armature circuit.
4. **DC Motor Electromechanical Plant**:
   * Electrical domain: Armature resistance ($R_a$), inductance ($L_a$), and back-EMF source ($E_b$).
   * Mechanical domain analog: Equivalent capacitance for rotor inertia ($J$) and resistance for viscous damping friction ($B$).
   * Disturbance modeling: Pulsed current injection source to simulate external mechanical load steps.
...
1. Open **Schematics** (PSpice / OrCAD).
2. Go to **File > Open** and select the desired schematic file from the `PSpice/` folder:
   * **`PID_MotorSpeed.sch`** to observe standard nominal step response.
   * **`PID_MotorSpeed_param.sch`** to run component sweeps.
